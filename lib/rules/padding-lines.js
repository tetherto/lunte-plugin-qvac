...
function expectedBlank(statements, i) {
  ...
  // a run of one-line statements, like an if-return ladder, reads as one group with its return
  const closing = EXITS.has(next.type) && i === statements.length - 1 && statements.length >= 3
  if (closing && statements.slice(0, i).some(spansLines)) {
